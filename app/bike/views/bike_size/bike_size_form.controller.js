...
(function() {
  ...
  angular
    .module('app.bike')
    .controller('BikeSizeFormController', BikeSizeFormController);

  BikeSizeFormController.$inject = ['$scope', 'bikeSize', '_DataService', 'toaster', '$state', '_Alert'];

  function BikeSizeFormController($scope, bikeSize, _DataService, toaster, $state, _Alert) {
    var vm = this;
    vm.bikeSize = bikeSize;
    vm.save = save;
    vm.remove = remove;
    activate();
    ...
    function save() {
      _DataService.save('bike-size', vm.bikeSize)
        .then(function(value){
          vm.bikeSize = value;
    ...
    function remove() {
      _DataService.remove('bike-size', vm.bikeSize)
        .then(function(value){
          toaster.pop('success', 'Succès', _Alert.getDefaultDeleteSuccess().message);
          $state.go('app.bike_size.list');
        })
        .catch(function(response){
          vm.alerts = [_Alert.getDefaultDeleteError(response.data)];
        });
