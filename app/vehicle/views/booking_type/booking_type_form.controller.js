...
(function() {
  ...
  angular
      .module('app.vehicle')
      .controller('BookingTypeFormController', BookingTypeFormController);

  BookingTypeFormController.$inject = ['$scope', 'bookingType', '_DataService', 'toaster', '$state', '_Alert'];

  function BookingTypeFormController($scope, bookingType, _DataService, toaster, $state, _Alert) {
    var vm = this;
    ...
    function save() {
      _DataService.save('booking-type', vm.bookingType)
        .then(function(value){
          vm.bookingType = value;
    ...
    function remove() {
      _DataService.remove('booking-type', vm.bookingType)
        .then(function(value){
          toaster.pop('success', 'Succès', _Alert.getDefaultDeleteSuccess().message);
          $state.go('app.vehicle.booking_type.list');
        })
        .catch(function(response){
          vm.alerts = [_Alert.getDefaultDeleteError(response.data)];
        });
