...
(function() {
  ...
  angular
    .module('app.vehicle')
    .controller('MaintenanceTypeFormController', MaintenanceTypeFormController);

  MaintenanceTypeFormController.$inject = ['$scope', 'maintenanceType', '_DataService', 'toaster', '$state', '_Alert'];

  function MaintenanceTypeFormController($scope, maintenanceType, _DataService, toaster, $state, _Alert) {
    var vm = this;
    ...
    function save() {
      _DataService.save('maintenance-type', vm.maintenanceType)
        .then(function(value){
          vm.maintenanceType = value;
    ...
    function remove() {
      _DataService.remove('maintenance-type', vm.maintenanceType)
        .then(function(){
          toaster.pop('success', 'Succès', _Alert.getDefaultDeleteSuccess().message);
          $state.go('app.vehicle.maintenance_type.list');
        })
        .catch(function(response){
          vm.alerts = [_Alert.getDefaultDeleteError(response.data)];
        });
