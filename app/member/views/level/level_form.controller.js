...
(function() {
  ...
  angular
    .module('app.member')
    .controller('LevelFormController', LevelFormController);

  LevelFormController.$inject = ['$scope', 'level', '_DataService', 'toaster', '$state', '_Alert'];

  function LevelFormController($scope, level, _DataService, toaster, $state, _Alert) {
    var vm = this;
    vm.level = level;
    vm.save = save;
    vm.remove = remove;
    ...
    function save() {
      _DataService.save('level', vm.level)
        .then(function(value){
          vm.level = value;
